-- Roadmap V3, Macro-release B (Lot 24) — enregistrer checklist_templates,
-- player_checklist_preferences et match_checklist_items dans export_backup_snapshot().
--
...
as $$
  select jsonb_build_object(
    ...
    'checklist_templates', coalesce((select jsonb_agg(to_jsonb(t) order by t.id) from public.checklist_templates t), '[]'::jsonb),
    'player_checklist_preferences', coalesce((select jsonb_agg(to_jsonb(t) order by t.id) from public.player_checklist_preferences t), '[]'::jsonb),
    'match_checklist_items', coalesce((select jsonb_agg(to_jsonb(t) order by t.id) from public.match_checklist_items t), '[]'::jsonb)
  );
$$;
