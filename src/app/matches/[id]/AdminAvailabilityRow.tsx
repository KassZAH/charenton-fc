...
export function AdminAvailabilityRow({
  ...
  return (
    <div className="flex items-center justify-between gap-2 rounded-lg border border-navy/10 bg-white px-3 py-2">
      <span className="text-sm text-navy">{playerName}</span>
      <select
        value={status ?? ""}
        disabled={isPending}
        onChange={(e) => change(e.target.value)}
        className="rounded-lg border border-navy/20 px-2 py-1 text-xs disabled:opacity-60"
      >
