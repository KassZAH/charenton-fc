...
# Macro-release Lots 8-11 (roadmap V3) — déployée en production, en attente de validation utilisateur finale
...
**Lot 8 — Transactions critiques restantes : déployé en production, en attente de validation utilisateur finale.** Complète la Phase 3.2 ci-dessus — `upsert_injury_and_sync_availability()` et `restore_audit_entry_transactional()`, deux RPC transactionnelles, appliquées au projet Supabase partagé. Corrige un écart de sécurité réel découvert pendant l'audit (restauration de fiche joueur qui exposait `pin_hash`/`session_version` à une réécriture silencieuse). 20/20 assertions de test vertes sur le projet isolé avant déploiement ; `EXECUTE` reconfirmé réservé à `postgres`/`service_role` sur le projet partagé après application.

**Lot 9 — Socle de tests d'intégration : déployé en production, en attente de validation utilisateur finale.** `scripts/isolated-env/`, suite `*.integration.test.ts` séparée, CI GitHub Actions — **réellement exécutée pour la première fois pendant ce déploiement**, ce qui a révélé et permis de corriger deux bugs d'infrastructure préexistants et jusque-là jamais détectés (lockfile incomplet pour Linux, variables d'environnement factices manquantes pour le build CI). Tests E2E navigateur délibérément différés.

**Lot 10 — Composants UI transversaux : déployé en production, en attente de validation utilisateur finale.** Six composants nouveaux (`ConfirmDialog`, `EmptyState`, `LoadingSkeleton`, `ErrorState`, `StatusBadge`, `ResponsivePageContainer`, `SectionAccordion`) intégrés dans un sous-ensemble représentatif (saisons, sauvegardes, fiche match, stats). `UndoToast` ajouté par extension du `ToastProvider` existant. `BottomSheet`/`AdminQuickActions` non créés faute d'usage réel identifié.

**Lot 11 — Responsive desktop et accessibilité : déployé en production, en attente de validation utilisateur finale.** `ResponsivePageContainer` appliqué aux 7 pages prioritaires restantes. `inputMode` ajouté sur les 14 champs numériques du dépôt. Audit exhaustif des cibles tactiles/`aria-describedby`/ordre de tabulation non repris dans ce lot — décision de périmètre assumée.
