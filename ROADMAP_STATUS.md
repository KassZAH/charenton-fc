...
Ce qui existe (déjà en production, `src/lib/data/seasons-actions.ts::startNewSeason` + `/admin/saisons`) : un mécanisme de transition de saison **déjà non destructif** — crée une sauvegarde (`end_of_season`), clôture et verrouille l'ancienne saison (`is_active=false, is_locked=true`), crée la nouvelle saison active. **Aucune donnée n'est supprimée** — exactement le principe demandé par la V2 §2.5 ("Aucune suppression de matchs n'est nécessaire pour commencer une nouvelle saison"). La page affiche déjà les matchs non terminés de la saison active avant la clôture (`getOpenMatchesInActiveSeason`) — couvre l'étape 1 du mini-assistant demandé. L'archivage de joueurs est disponible sur la même page (étape 9, mais en libre-service, pas intégré au flux de soumission du formulaire).

Ce qui manque par rapport aux 11 étapes détaillées en V2 §2.5 : afficher les incohérences au-delà des matchs non terminés (étape 2) ; clôturer/ignorer les votes ouverts (étape 3 — aucune notion de vote "ouvert/fermé" n'existe, voir Phase 11.7) ; générer le bilan de saison dans le même flux (`/season-recap` existe déjà comme page séparée, pas branchée à l'assistant) ; générer la chronique dans le même flux (la frise de `/memoire` existe déjà, pas branchée) ; choix explicite du traitement des cotisations (aucune notion de report/remise à zéro des `dues` par saison).

Fichiers concernés : `src/app/admin/saisons/page.tsx`, `src/lib/data/seasons-actions.ts`, `src/lib/data/season-recap.ts`, `src/lib/data/club-memory.ts`, `src/lib/data/dues.ts`.
Tables/migrations : aucune nouvelle nécessaire pour relier les pages existantes ; une colonne `season_id` sur `dues` serait nécessaire pour un vrai traitement par saison des cotisations (à vérifier — actuellement `dues` n'a probablement pas de scope saison, à confirmer avant de coder).
Dépendances techniques : Phase 11.7 (cycle de votes) pour l'étape 3 complète.
Risque sur données existantes : faible — le socle actuel ne supprime rien ; étendre l'assistant ne fait qu'ajouter des étapes d'affichage/liens.
Effort estimé : **M** (relier bilan + chronique + incohérences dans le même écran), **S** additionnel par sous-étape.
Tests déjà présents : aucun automatisé (vérifié manuellement : anciennes saisons intactes après clôture, saison active unique).
Tests à ajouter : clôture n'efface aucune donnée historique ; une seule saison active à la fois (contrainte SQL manquante, voir Phase 3.1) ; propriétaire requis (dépend de 1.1, actuellement `requireAdmin` suffit).
Migration nécessaire : possiblement pour `dues.season_id` si retenu.
Déployable indépendamment : oui, par sous-étape.
Feature flag léger : non nécessaire.
...
Ce qui existe : deux RPC transactionnelles livrées ce jour — `export_backup_snapshot()` (Phase 2.2) et `confirm_match_roster()` (remplacement de feuille de match, migration `20260718020000_transactional_roster.sql`, atomique : suppression + réinsertion dans un seul corps de fonction PL/pgSQL). `duplicateMatch` (`src/lib/data/matches-actions.ts`) a reçu une mitigation non transactionnelle mais équivalente en pratique : si une étape échoue après la création du match, le match orphelin est supprimé en compensation (`try/catch` + `delete`) — ce n'est pas une vraie transaction Postgres, mais ça atteint le même objectif pratique (jamais de match "fantôme" à moitié dupliqué).

Ce qui manque : RPC transactionnelles pour clôture de blessure + mise à jour des disponibilités (`injuries-actions.ts` reste multi-étapes, idempotent mais pas transactionnel), restauration d'une entrée audit (`audit-actions.ts::restoreChange`, deux étapes : mutation + marquage `restored_at`), clôture de saison (actuellement 3 opérations séparées dans `startNewSeason`, pas de rollback si l'insertion de la nouvelle saison échoue après la clôture de l'ancienne), fusion de joueurs (fonctionnalité qui n'existe pas encore, voir Phase 8.5), fin de match et création des événements associés (dépend de la Phase 5, pas encore construite).
