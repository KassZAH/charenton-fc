...
# Macro-release Lots 8-11 (roadmap V3) — terminée et validée en production

_Branche `macro-8-11-consolidation` fusionnée sur `master` en deux vagues fast-forward (Vague A : Lots 8-9 + 2 correctifs d'infrastructure CI nécessaires ; Vague B : Lots 10-11). Backup protégé `before_macro_8_11_production` créé avant toute action. Migration Lot 8 appliquée au projet Supabase partagé après reconfirmation exhaustive. Aucune mutation métier réelle (aucune blessure, aucun archivage, aucune restauration, aucune clôture de saison) exécutée pendant le déploiement — voir `roadmap-v3-discussion/macro-8-11-consolidation/` pour le détail complet._

**Lot 8 — Transactions critiques restantes : terminé et validé en production.** Complète la Phase 3.2 ci-dessus — `upsert_injury_and_sync_availability()` et `restore_audit_entry_transactional()`, deux RPC transactionnelles, appliquées au projet Supabase partagé. Corrige un écart de sécurité réel découvert pendant l'audit (restauration de fiche joueur qui exposait `pin_hash`/`session_version` à une réécriture silencieuse). 20/20 assertions de test vertes sur le projet isolé avant déploiement ; `EXECUTE` reconfirmé réservé à `postgres`/`service_role` sur le projet partagé après application.

**Lot 9 — Socle de tests d'intégration : terminé et validé en production.** `scripts/isolated-env/`, suite `*.integration.test.ts` séparée, CI GitHub Actions — **réellement exécutée pour la première fois pendant ce déploiement**, ce qui a révélé et permis de corriger deux bugs d'infrastructure préexistants et jusque-là jamais détectés (lockfile incomplet pour Linux, variables d'environnement factices manquantes pour le build CI). Tests E2E navigateur délibérément différés.

**Lot 10 — Composants UI transversaux : terminé et validé en production.** Six composants nouveaux (`ConfirmDialog`, `EmptyState`, `LoadingSkeleton`, `ErrorState`, `StatusBadge`, `ResponsivePageContainer`, `SectionAccordion`) intégrés dans un sous-ensemble représentatif (saisons, sauvegardes, fiche match, stats). `UndoToast` ajouté par extension du `ToastProvider` existant. `BottomSheet`/`AdminQuickActions` non créés faute d'usage réel identifié.

**Lot 11 — Responsive desktop et accessibilité : terminé et validé en production.** `ResponsivePageContainer` appliqué aux 7 pages prioritaires restantes. `inputMode` ajouté sur les 14 champs numériques du dépôt. Audit exhaustif des cibles tactiles/`aria-describedby`/ordre de tabulation non repris dans ce lot — décision de périmètre assumée.

**Déploiement production vérifié** : Vague A (commit `7935bab`, inclut les 2 correctifs CI) et Vague B (commit `1c1dc80`) tous deux confirmés `target: production`/`READY` sur les commits exacts, CI verte sur les deux, données métier reconfirmées strictement inchangées après chaque vague (24 joueurs, saison réelle active inchangée, 30 matchs, 9 dues, seul `backups` passé de 5 à 6 pour le backup pré-déploiement).
...
Deux correctifs de sécurité/complétude trouvés et corrigés pendant la vérification pré-déploiement, avant toute exposition prolongée : Row Level Security n'était pas activée sur les trois nouvelles tables (contrairement à toutes les autres tables du schéma), permettant à la clé `anon` publique de lire/écrire directement dessus via PostgREST — corrigé (migration `20260721000200`), vérifié en direct (anon refusé après correctif). Les trois tables étaient aussi absentes du registre de sauvegarde (`BACKUP_TABLES`/`export_backup_snapshot()`), détecté par le test de couverture du Lot 6 — corrigé (migration `20260721000300`).

Le classement réel de la compétition ciblée (championnat 13, saison 2) est actuellement **vide** (aucune équipe inscrite pour cette saison sur le site source) — confirmé par un vrai appel réseau en direct contre la source réelle lors de la validation en preview, jamais une erreur ni un classement fabriqué. La compétition réelle est configurée en production (`sync_enabled=true`), aucune donnée fictive. Deux modes de test restent disponibles sur le projet isolé uniquement : réel vide (`scripts/isolated-env/reset-and-seed.js`) et démonstration fictive (`scripts/isolated-env/load-fla-demo-fixtures.js`).
...
Tables/migrations : quatre migrations additives, appliquées au projet Supabase partagé le 19/07/2026 après backup protégé préalable (`before_lot_11_5_fla_standings`).
Tests : 24 tests unitaires (parsing contre deux pages HTML réelles sauvegardées, normalisation, association, résolution d'affichage) + 10 tests d'intégration sur le projet isolé (transactionnalité de la RPC, préservation du cache, refus anon) — gate complète (`npm ci`/`tsc`/`lint`/`test`/`test:integration`/`build`) verte avant et après les deux correctifs de sécurité.
Migration nécessaire : non, déjà appliquée.
Déployable indépendamment : oui — déployé.
...
Fusion fast-forward `lot-11-5-fla-standings` → `master` (commit `696d26b`), CI verte, déploiement Vercel `target: production`/`READY` confirmé sur le commit exact — voir `roadmap-v3-discussion/lot-11-5-fla-standings/` pour le rapport complet.
